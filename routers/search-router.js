...
router.get('/by-last-name/:lastName', async (req, res) => {
    ...
    const person = await personRepository.search().where("lastName").equals(lastName).return.all()
    res.send(person)
})
...
router.get('/old-enough-to-drink-in-america', async (req, res) => {
  ...
  router.get('/verified-drinkers-with-last-name/:lastName', async (req, res) => {
    ...
    const lastName = req.params.lastName
const person = await personRepository.search().where("verified").is.true().and("age").gte(21).and("lastName").equals(lastName).return.all()
...
res.send(person)
  ...
  router.get('/near/:lng,:lat/radius/:radius', async (req, res) => {
    ...
    const person = await personRepository.search().where("location").inRadius( circle => circle.longitude(longitude)
    .latitude(latitude)
